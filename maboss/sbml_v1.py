...
# def getInputs(formula):
#
# 	if '?' in formula and ":" in formula:
# 		res_match = re.match("(.+)\?(.*):(.*)", model.network['CycA'].rt_up)
# 		(formula, true, false) = tuple([element.strip() for element in res_match.groups()])
# 		if '@logic' in formula:
# 			formula = formula.replace('@logic', species.logExp)
# 		#print(formula)
...
#
# 	inputs = getInputs(species.rt_up)
# 	for input in inputs:
...
#
# 	formula, true, false = parseFormula(species.rt_up)
# 	print("")
# 	print(formula)
# 	print(libsbml.formulaToString(maboss.booleanToSBML(algebra.parse(formula))))
...
def addReaction(sbml_model, sid, species):
	...
	# formula, true, false = parseFormula(species.rt_up, species)
	# print("Uprate formula : %s" % formula)
	# boolean_formula = algebra.parse(formula.replace("$", "__"))
	# up_formula = libsbml.ASTNode()
	# if isValue(formula):
	# 	up_formula.setType(libsbml.AST_REAL)
	# 	up_formula.setValue(float(formula))
	#
	# else:
	# 	up_formula.setType(libsbml.AST_FUNCTION_PIECEWISE)
	# 	up_formula.addChild(addRateValue(true))
	# 	up_formula.addChild(booleanToSBML(boolean_formula))
	# 	up_formula.addChild(addRateValue(false))

	# formula, true, false = parseFormula(species.rt_down, species)
	# print("Downrate formula : %s" % formula)
	# boolean_formula = algebra.parse(formula.replace("$", "__"))
	#
	# down_formula = libsbml.ASTNode()
	# if isValue(formula):
	# 	down_formula.setType(libsbml.AST_REAL)
	# 	down_formula.setValue(float(formula))
	#
	# else:
	# 	down_formula.setType(libsbml.AST_FUNCTION_PIECEWISE)
	# 	down_formula.addChild(addRateValue(true))
	# 	down_formula.addChild(booleanToSBML(boolean_formula))
	# 	down_formula.addChild(addRateValue(false))
	# sbml_formula.addChild(down_formula)
